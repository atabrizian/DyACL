 ...
 * data for our test.
 *
 * You should change database connection info according to your own database connection
 * info
 *
 * Before running the test i want to explain the sample data - which are accessible from
 * 'sample.sql' - to you.
 *
 ...
 *
 * row 3: id:3 role_id:2 reource_name:user_can_only_view action:view status:allow
 * It means: 'User' role can just view the resource 'user_can_only_view'
 *
 ...
 * except deleting it or whatever the action 'delete' may mean.
 *
...
echo "Checking 'user_can_only_view':<br>";

if ($dyAcl->isAllowed('user_can_only_view')) {
    echo "Yes, 'all' action is allowed.<br>";
} else {
    echo "No, 'all' action is not allowed.<br>";
}

if ($dyAcl->isAllowed('user_can_only_view', 'view')) {
    echo "Yes, view is allowed.<br>";
} else {
    echo "No, view is not allowed.<br>";
}
...
if ($dyAcl->isAllowed('user_can_not_delete', 'view')) {
    echo "Yes, view is allowed.<br>";
} else {
    echo "No, view is not allowed.<br>";
}

if ($dyAcl->isAllowed('user_can_not_delete', 'delete')) {
